...
### 「3分」を探した検索式を、そのまま出します

「ちゃんと探したのか」と思われるはずなので、投げた検索を2つとも出します。

**1つめ。「3分」という数字を、そのまま探しました。**

```
("three minute"[tiab] OR "3 minutes"[tiab] OR "3-minute"[tiab])
  AND (moisturizer*[tiab] OR emollient*[tiab]) AND (bath*[tiab] OR shower*[tiab])
```

**返ってきたのは1件。0件ではありません。** ですが、その1本は**3分ルールの話ではありませんでした。** アトピー性皮膚炎に使う「乳化入浴剤」の研究です。「3分」と「入浴」と「保湿剤」という語が、たまたま同じ論文に入っていただけでした。

**「風呂上がり3分以内」という数字を検証した論文は、この検索では1本も出てきませんでした。**

**2つめ。数字ではなく、中身で探しました。「保湿剤を塗るタイミング」の研究です。**

```
(moisturizer[tiab] OR emollient[tiab] OR "moisturizing cream"[tiab])
  AND (timing[tiab] OR "immediately after"[tiab] OR delayed[tiab] OR interval[tiab])
  AND (bath[tiab] OR bathing[tiab] OR shower[tiab]) AND skin[tiab]
```

**返ってきたのは16件です。0件ではありません。塗るタイミングを調べた研究は、確かに存在します。**

**ただし、16本の中に「3分」を試したものは、1本もありませんでした。** 実際に比べられていたのは、**直後 vs 30分後**、**直後 vs 90分後**。この記事で紹介する試験も、そこから出てきたものです。**誰も「3分」を測っていない。測られているのは、もっと大きな時間の幅でした。**

そして、その比較の答えは「有意差なし」でした。次に見ます。

この2つの検索は、**記事の末尾から、あなたの画面で再実行できます。** 1件と16件が返ってくること、その中に「3分」を試した研究が無いことを、ご自分で確かめてください。**私を信じる必要はありません。**
