...
PubMed（世界の医学論文のデータベース）に、2つの検索を投げました。両方とも、そのまま出します。
...
**返ってきたのは0件でした。** 1本もありません。世界の医学論文のタイトルと要旨のどこにも、この言葉は出てきませんでした。**これは、この記事でいちばん静かで、いちばん強い結果です。**

**2つめ。言葉ではなく、中身で探しました。「保湿剤の使用を中止する」という介入の臨床試験です。**

```
(moisturizer[tiab] OR emollient[tiab] OR moisturiser[tiab])
  AND (discontinuation[tiab] OR cessation[tiab] OR withdrawal[tiab] OR abstinence[tiab])
  AND skin[tiab] AND ("clinical trial"[pt] OR "randomized controlled trial"[pt])
```

**返ってきたのは8件。0件ではありません。ただし、8本とも「スキンケアをやめる」話ではありませんでした。**

中身は、**光線療法の話、ステロイド外用薬の話、放射線による皮膚炎の話**です。医療の現場で「塗り薬をやめる」場面の研究であって、化粧品をやめる話ではありません。「保湿剤」と「中止」という語が、たまたま同じ論文に入っていただけです。

正直に、もう1つ書いておきます。**この検索式では、次に紹介する2020年の研究（顔の片側だけ保湿剤をやめた試験）も返ってきませんでした。** 別の探し方で見つけたものです。つまり、**この8件は「世界のすべて」ではありません。こちらの検索が下手だった可能性は、常に残ります。** だから検索式を出します。もっと上手な式を思いついたら、あなたのほうが先に見つけられます。

この2つの検索は、**記事の末尾から、あなたの画面で再実行できます。** 0件と8件が返ってくること、その8件がスキンケアの話ではないことを、ご自分で確かめてください。**私を信じる必要はありません。**
