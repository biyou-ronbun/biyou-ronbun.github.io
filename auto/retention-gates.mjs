...

{
  ...
  //
  //   診断ページには、こう書いてある:
  //     「『あなたの肌は敏感肌です』のような判定は**しません**。それは医師の仕事です」
  //
  //   最初これを違反として拾った。**関門が、自分の善行を違反と判定した。**
  //   （この壊れ方は、うちの関門で何度も起きている）
  const DENIAL = ['しません', 'ではありません', '言えません', '書きません', '判定は', '禁止'];

  // 医師法17条: 肌の状態を医学的に判定していないか
  const DIAGNOSIS = /あなたの肌は[^。]{0,12}(です|でしょう)|炎症があります|と診断されました/;

  const files = existsSync(TPL) ? readdirSync(TPL) : [];
  for (const f of files) {
    for (const line of read(join(TPL, f)).split(/\r?\n/)) {
      if (!DIAGNOSIS.test(line)) continue;
      if (DENIAL.some((d) => line.includes(d))) continue; // 打ち消している文は見逃す
      failures.push(
        `templates/${f}: ★★ 肌の状態を、医学的に判定しています【医師法17条】\n` +
