...
// 他メディアの型。**入れた瞬間、このサイトは別物になる。**
const REJECTED = [
  { re: /ランキング|人気順|おすすめ順|総合評価|ベスト\s*\d|第\s*\d+\s*位/, why: 'ランキング（@cosme / LIPS の中核）。判定は資産になり、最後に売られる' },
  // ★ 2026-07-13 オーナー判断: 「成分辞典」と「商品データベース」の**却下を解除**。
  //
  //   ただし、**危険だった部分は解除していません。**
  //
  //   却下の理由は「辞典」という名前ではありませんでした。**中身でした。**
  //     ・解説を書くこと      → うちの言葉で成分を語れば、それは論文ではなく「意見」
  //     ・判定を持つこと      → 順位・点数・おすすめ
  //
  //   だから、**語ではなく中身で止めます。**
  //     ・順位・点数・星         … 下の行で、いまも止まる
  //     ・おすすめ・ベスト・1位  … 下の行で、いまも止まる
  //     ・診断→おすすめ成分      … 却下のまま（検証した試験 0件）
  //
  //   **「成分辞典」を作ってよい。ただし、判定を持てば、いまも公開が止まる。**
  { re: /この成分は(効き|おすすめ|向いて)/, why: '成分に「判定」を書いている。辞典が判定を持った瞬間、それは資産になり、最後に売られる' },
  // ★ 「★」そのものは検出しない。**うちは ★ を強調記号として全ファイルで使っている。**
  //   （最初これを星の評価と誤判定した。関門は、自分の善行を違反と判定しがち）
  { re: /総合スコア|評価点|星\s*\d\s*つ|\d+\s*点満点|\d+\s*点\s*\/\s*\d+|グレード\s*[A-E]\b|5段階評価/, why: '点数・星・グレード。読者を採点する装置' },
  { re: /バッジ|レベルアップ|連続記録|ログインボーナス|ポイントを獲得/, why: 'ゲーミフィケーション。採点した瞬間「騙されてましたね」に変わる' },
  { re: /メールでお届け|メールマガジン|メールで受け取|メール会員/, why: 'メール登録。**届ける手段が無い。** 続かない約束を置いた時点で、唯一の武器が壊れる' },
  { re: /通知を(許可|オン|受け取)/, why: '通知をねだる。通知は追跡の入口' },
  { re: /毎日更新中|毎日更新！|更新中!/, why: '「また来てください」と言って人は来ない' },
  { re: /[\d,]+\s*人が(読者|購読|支援|登録)/, why: '★ 読者数の捏造。購読者は 0 人' },
];
