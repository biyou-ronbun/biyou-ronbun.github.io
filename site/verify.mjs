...
//
// ★ 2026-07-14、オーナー判断でタイアップを開けました。**開示は、外せません。**

{
  const { SPONSOR_REQUIRED } = await import('./sponsor.mjs');

  for (const a of meta) {
    const s = a.sponsor;
    if (!s) continue;

    // ① 開示に必要な項目が、全部そろっていること
    for (const k of SPONSOR_REQUIRED) {
      if (!s[k]) {
        failures.push(
          `${a.slug}: **タイアップ記事なのに「${k}」が空です**\n` +
            `      必要なのは: ${SPONSOR_REQUIRED.join(' / ')}\n` +
            `      **誰から / 何を約束したか / 何を約束していないか。** これを書かないのは、景表法違反です`
        );
      }
    }

    // ② 公開された HTML に、開示が実際に出ていること
    const html = join(SITE, 'dist', 'articles', `${a.slug}.html`);
    if (existsSync(html)) {
      const h = readFileSync(html, 'utf8');
      if (!h.includes('PR（タイアップ記事）')) {
        failures.push(
          `${a.slug}: **タイアップ記事なのに、公開ページに開示が出ていません**\n` +
            `      **景品表示法のステマ規制。罰せられるのは、表示した側です。**`
        );
      }
      if (!h.includes(s.company)) {
        failures.push(
          `${a.slug}: **タイアップ記事なのに、公開ページに「${s.company}」の名前が出ていません**\n` +
            `      **誰から金を受け取ったかを、実名で書くこと。**`
        );
      }
    }

    // ③ ★ 結論を売っていないか
    //   「必ず良い結論を書く」と約束したら、それはもう検証ではありません。
    const SOLD = /良い結論|好意的な内容|効果があると書|推薦すること|おすすめすること/;
    if (SOLD.test(s.promised ?? '')) {
      failures.push(
        `${a.slug}: **結論を売っています**（約束したこと: 「${s.promised}」）\n` +
          `      **結論を約束した瞬間、この記事は検証ではなく、広告そのものです。**\n` +
          `      約束してよいのは「記事を書くこと」までです。**結論は、論文が決めます。**`
      );
    }
  }
}

// ---- ランキング: 重みを表示せずに、順位を出していないか -----------------------
//
// ★★ 2026-07-14、オーナー判断でランキングを開けました。
//
//   **順位は、論文が決めたものではありません。私たちが決めた重みで計算したものです。**
//
//     ・なぜ「独立資金の試験」に +1 なのか  → **私たちがそう決めたから**
//     ・なぜ「価格」が最大 2点 なのか        → **私たちがそう決めたから**
//     ・なぜ「撤回された論文」が −1 なのか    → **私たちがそう決めたから**
//
//   **重みを表示せずに順位だけを出すと、読者は「科学が1位だと言っている」と読みます。**
//   **それは嘘です。**
//
//   だから: **順位を出すページには、必ず重みの開示があること。**
//
// ★ この検査を外したくなったら、それが危険信号です。
//   「重みを見せると、読者が混乱する」——そう思った瞬間、
//   このサイトは「なんとなく1位だから買う」を作る装置になります。

{
  const distDir = join(SITE, 'dist');
  if (existsSync(distDir)) {
    // 順位を名乗っている表現
    const SHOWS_RANK = /第?\s*\d+\s*位|1位|総合スコア|ランキング/;
    // 重みを開示している合図（この文字列が無ければ、開示していないとみなす）
    const DISCLOSES = '私たちが決めた重み';
    ...
    for (const file of walk(distDir)) {
      ...
      if (!SHOWS_RANK.test(html)) continue;
      if (html.includes(DISCLOSES)) continue;
      failures.push(
        `${file.replace(distDir, '')}: **順位を出しているのに、重みを開示していません**\n` +
          `      順位は、論文が決めたものではありません。**私たちが決めた重みで計算したものです。**\n` +
          `      重みを見せずに順位だけを出すと、読者は「科学が1位だと言っている」と読みます。**それは嘘です。**\n` +
          `      ページに「私たちが決めた重み」の説明（重みの表）を出すこと`
      );
    }
  }
}

// ---- メンバーシップ: 約束したものが、実在するか ----------------------------
//
// ★★ CLAUDE.md:「★ すでに一度、壊れた」
//
...
//
// ★ 2026-07-14、同じことがまた起きかけていた。
//
//   メンバーページ（購読 980円）は、3つ約束している:
//       ① 公開前の記事 — 公開日の3日前から読めます
//       ② 記事にならなかった検証メモ
//       ③ 次に調べるテーマへの投票
//
//   実測すると、① は 0本、② は 0件、③ は「準備中です」だった。
//   **記事の輪が「今日の日付」で記事を出すので、先出しが一度も発生していない。**
//   **払った人が見るページの、半分が空になる。**
//
// ★ だから: **金の出口（支払いリンク）が開いているなら、約束したものが在ること。**
//   リンクが無いうちは、メンバーシップ自体が出ないので、検査しない。

{
  const cfgFile = join(SITE, 'config.json');
  const cfg = JSON.parse(readFileSync(cfgFile, 'utf8'));
  const mem = cfg.membership ?? {};
  const paidPlans = (mem.plans ?? []).filter((p) => p.url);

  // ★ 支払いリンクが1本でも入った ＝ 金を取り始める。そのときだけ検査する。
  if (paidPlans.length) {
    const today = new Date(Date.now() + 9 * 3600_000).toISOString().slice(0, 10);

    // 「購読」プラン（メンバーページが付く方）が売られているか
