...

{
  ...
  if (existsSync(newsFile)) {
    ...
    const SOURCE_VOCAB = [
      ...
      '厚労省・消費者庁', // ★ 新しく増やさない（下記）
    ];

    // ★ 「厚労省・消費者庁」は、**今後もう増やしません**（オーナー判断・2026-07-13）。
    //
    //   ただし語彙からは消しません。**すでに公開中の2件が、この出どころだからです。**
    //   語彙から消せば、その2件が公開できなくなり、**過去の記事が消えます。**
    //   （東京都の措置命令の記事は、うちの中でも特に強い1本です）
    //
    //   **消さずに、増やさない。** それを機械で担保します。
    const FROZEN_SOURCE = '厚労省・消費者庁';
    const FROZEN_ALLOWED = ['2026-07-13-glp1-slimming', '2026-07-13-whitening-serum-order'];

    for (const n of items) {
      if (n.source === FROZEN_SOURCE && !FROZEN_ALLOWED.includes(n.id)) {
        failures.push(
          `ニュース ${n.id}: source が「${FROZEN_SOURCE}」です。\n` +
            `      **この出どころは、もう増やしません**（オーナー判断・2026-07-13）。\n` +
            `\n` +
            `      すでに公開中の2件のために語彙としては残していますが、\n` +
            `      **新しいニュースをここに入れないでください。**\n` +
            `\n` +
            `      いま追うのは「企業の発表」（新発売・新成分）です。`
        );
      }
    }
