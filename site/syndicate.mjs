...
// ---- 読みやすく改行する ---------------------------------------------
//
// ★ note / はてな は、スマホで読まれます。
//   1段落が200字を超えると、**画面が文字で埋まって、読まれません。**
//   紙の文章と、スマホの文章は、改行の作法が違います。
//
//   だから、文の切れ目（。）で区切って、短い塊にします。
//
// ★ 触ってはいけないもの:
//   ・箇条書き（- で始まる行）… 崩れる
//   ・コードブロック（``` で囲まれた部分）… 検索式が壊れる
//   ・見出し（# で始まる行）
//   ・「」の中（引用の途中で切ると、意味が変わる）
...
const breakParagraphs = (text) => {
  ...
  for (const block of text.split(/\n\n+/)) {
    ...
    // 触らないもの
    if (/^[#\->|]/.test(b) || b.includes('\n- ') || b.startsWith('- ') || b.length <= MAX_PARA) {
      out.push(b);
      continue;
    }
    ...
    // ★ 切ってはいけない場所が2つある:
    //   ・「」『』の中 … 引用の途中で切ると、意味が変わる
    //   ・**強調** の中 … **の途中で切ると、Markdown が壊れて ** が本文に出る
    //     （実際にこれをやって、「**1. …ました。」で切れて ** が孤立した）
    const parts = [];
    let cur = '';
    let quote = 0;
    let bold = false;
...
for (const a of meta) {
  ...
  daily.text = breakParagraphs(daily.text);

  // ---- ここまでは言えません（このブログの本体。ただし全部は出さない） ----
  const limits = trim(section(md, /ここまでは言えません/), 700);
  limits.text = breakParagraphs(limits.text);
...
**PubMed**（パブメド）という、世界中の医学論文が集まっているデータベースがあります。

**無料で、誰でも使えます。**

この記事を書くとき、機械が実際に投げた検索が、これです。

**押せば、あなたの画面で同じ検索が走ります。**
...
増えていたら、それは私たちが記事を更新すべきだというサインです。
`
    : '';
...
${breakParagraphs(trim(answerBody, 700).text)}
...
**1本でも通らなければ、その記事は公開されません。**

この記事は、${v.count ?? '?'} 本の論文すべてが、この検査を通っています。

ただし、正直に書いておきます。

**確かめられるのは、論文が実在することだけです。**

論文の中身が正しいかどうかまでは、機械には分かりません。**医師の監修もありません。**
...
引用した論文 ${v.count ?? '?'} 本の一覧、数値をグラフにした図、「ここまでは言えません」の全文も、そこにあります。
...
- **「肌のターンオーバーは28日」**
　→ その28日を測った論文が、見つかりませんでした

- **「飲んだヒアルロン酸が肌に届く」**
　→ 出典をたどったら、ラットの呼気にたどりつきました

- **「コラーゲンサプリの研究結果が割れている」**
　→ 分けていたのは、誰がお金を出したかでした

**効果は約束しません。**

渡すのは商品ではなく、**広告を自分で読み解ける目**です。

平日は毎日、1本ずつ調べています。

**「根拠が見つかりませんでした」で終わることもあります。** それも、そのまま書きます。
